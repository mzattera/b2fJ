...
public class TinyVMTool extends AbstractTool {
	 ...
	 * @param all
	 * @param stream
	 *            output stream to write binary to
	 * @param bigEndian
	 *            write big endian output?
	 * @throws TinyVMException
	 */
	public void link(String classpath, String[] classes, boolean all, OutputStream stream, boolean bigEndian)
			throws TinyVMException {
		...
		for (int i = 0; i < entryClassNames.length; i++) {
			if (!result.hasMain(entryClassNames[i])) {
				throw new TinyVMException(
						"Class " + entryClassNames[i] + " doesn't have a static void main(String[]) method");
			}
		}
	...
	public void dump(Binary binary, OutputStream stream, boolean bigEndian) throws TinyVMException {
		...
		IByteWriter byteWriter = bigEndian ? new BECCodeWriter(bufferedStream) : new LECCodeWriter(bufferedStream);
		try {
