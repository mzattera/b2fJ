 ...
 * @author Massimiliano 2maxi" Zattera
 * 
 * @since JDK1.1
 */
...
public abstract class Reader implements Readable, Closeable {
	 ...
	 * character-stream object may use an object other than itself to protect
	 * critical sections. A subclass should therefore use the object in this field
	 * rather than <tt>this</tt> or a synchronized method.
	 */
	 ...
	 * @since 1.5 public int read(java.nio.CharBuffer target) throws IOException {
	 *        int len = target.remaining(); char[] cbuf = new char[len]; int n =
	 *        read(cbuf, 0, len); if (n > 0) target.put(cbuf, 0, n); return n; }
	 */
	...
	public int read(char cbuf[], int off, int len) throws IOException {
		synchronized (this.lock) {
			...
			for (i = off; i < (off + len); ++i) {
				...
				char c = (char) ((InputStream) this.lock).read();
